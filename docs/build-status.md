...
- 93 offline tests pass: policy, transport, connection response handling, frozen calls, journal recovery, same-NFT return, testnet chain/simulation boundaries, single-submission protection and sponsored receipt verification. Three optional live tests are skipped in the offline suite; they passed separately during the initial read milestone.
...
KeeperHub authentication and simulation pass. **The first real KeeperHub testnet value-movement transaction is verified:** 0.001 test ETH wrapped into WETH on Base Sepolia, execution `chodgtho78elwzx45xc28`. See [testnet receipt and scope](testnet.md). The public testnet Uniswap/Aave lifecycle is still pending. The project execution wallet had no Base mainnet ETH/WETH/USDC or Uniswap NFT at the last recorded mainnet read. The deployed console still represents the earlier read/fork milestone; its runtime has not received either local API credential.
...
1. The user authorized Read + Write access and the initial 0.001 test ETH deposit completed on Base Sepolia (84532). Create the project-owned Uniswap NFT from the resulting WETH; no additional initial funding is needed. See [testnet status](testnet.md). No mainnet funding or execution is currently authorized.
