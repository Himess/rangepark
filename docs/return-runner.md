...
- No hosted scheduler or automatic dispatch. The [continuous local monitor](return-monitor.md) refreshes economics and observations while its process runs, but does not export an executable quote or invoke this runner. The separate [economics generator](return-economics.md) supplies the runner’s expiring local quote. Initial economics must be current before withdrawal; subsequent steps enforce price/account/capital guards rather than forecasting profitability again.
